...
const char program_name[] = "ffplay_ultimate_edition";
...
void do_norm_calc(int ox, int oy, int width, int height, Frame *vp, EBAL *normalizer,AVPicture *pict);
void do_norm_do(int ox, int oy, int width, int height, Frame *vp,AVPicture *pict, EBAL *normalizer1, EBAL *normalizer2, EBAL *normalizer3, EBAL *normalizer4);                  
...
uint8_t *screenbar=NULL;
int screenbar_width=0;
...
uint8_t *vsrat_font_offset[FONT_CHARS]={0};
int vsrat_font_len[FONT_CHARS];

void podpisat(AVPicture *pict, Frame *vp, char *text, int posY);
char *vsrat_get_time(VideoState *cur_stream, AVFrame *vp);
void srat_l(HKEY k, char* subkey, char* name, char *value);
char *sosat_t(HKEY hKey, char *strValueName);
...
static void blend_subrect(AVPicture *dst, const AVSubtitleRect *rect, int imgw, int imgh)
{
    ...
    for (y = 0; y<dsth/2; y++) {
        for (x = 0; x<dstw/2; x++) {
            U = src->data[1][x + y*src->linesize[1]]*3;
...
v=pict->data[0][ox+q+(oy+w)*vp->bmp->pitches[0]];
...
if(is_char){
vsrat_font_offset[n]=vsrat_font+offset+FONT_WIDTH;
vsrat_font_len[n]=q-offset;
is_char=0;
n++;
}
}
...
if(vsrat_font_offset[0]==0 || vsrat_font_len[0]==0){font_init();}
...
draw_size+=vsrat_font_len[charID]+1;
}


posX=vp->width-draw_size;
for(e=0;e<len;e++){
if(text[e]==' '){posX+=10;continue;}
charPos=strchr(chars,text[e]);
if(!charPos){continue;}
charID=charPos-chars;
if(charID<0 || charID>=FONT_CHARS){continue;}
charFont=vsrat_font_offset[charID];


if(posX>=0){
canvas=&pict->data[0][posX+posY*vp->bmp->pitches[0]];
for(w=0;w<FONT_HEIGHT;w++){
if(w+posY>vp->height){continue;}
for(q=0;q<vsrat_font_len[charID];q++){
*canvas++=*canvas<*charFont?*charFont:*canvas;
charFont++;
}
charFont+=FONT_WIDTH-vsrat_font_len[charID];
canvas+=vp->bmp->pitches[0]-vsrat_font_len[charID];
}
}
posX+=1+vsrat_font_len[charID];
}
...
int scene=detector_frame(pict.data[0],vp->width,vp->height);
...
if(scene>200){
do_norm_calc(0,0,vp->width,vp->height,vp,normalizers[0],&pict);
...
do_norm_calc(q-step,w-step,step*3,step*3,vp,normalizers[i++],&pict);
}
}
...
do_norm_do(q*step,w*step,step,step,vp,&pict,normalizers[i],normalizers[i+1],normalizers[i+h_count],normalizers[i+h_count+1]);                         
} else if(q+1<h_count){
do_norm_do(q*step,w*step,step,step,vp,&pict,normalizers[i],normalizers[i+1],normalizers[i],normalizers[i+1]);                         
} else if(w+1<v_count){
do_norm_do(q*step,w*step,step,step,vp,&pict,normalizers[i],normalizers[i],normalizers[i+h_count],normalizers[i+h_count]);                         
} else {
do_norm_do(q*step,w*step,step,step,vp,&pict,normalizers[i],normalizers[i],normalizers[i],normalizers[i]);                         
}
i++;
}
}
...
if(vp->width!=screenbar_width){
if(screenbar){free(screenbar);}
screenbar_width=vp->width;
screenbar=malloc(screenbar_width*10);
...
max=0;
for(q=0;q<1000;q++){
if(bitbar[q]>max){max=bitbar[q];}
}

for(q=0;q<1000;q++){
i=q*vp->width/1000;
for(w=0;w<10;w++){
screenbar[i+w*screenbar_width]=bitbar[q]*255/max;
}
}
}


memcpy(&pict.data[0][0],screenbar,screenbar_width*10);


i=is->audio_clock*vp->width/(is->ic->duration / 1000000LL);
for(w=0;w<15;w++){
pict.data[0][i+w*vp->bmp->pitches[0]]=255;
pict.data[0][i+1+w*vp->bmp->pitches[0]]=0;
}
posY+=15;
}
...
memset(stream, 0, len);
    ...
    while (len > 0) {
        if (is->audio_buf_index >= is->audio_buf_size) {
           ...
           if (vsrat_fadein--==100 || vsrat_sound || audio_size < 0) {
...
if(vsrat_fadein){
Compressor_reset();
printf("resetting by fadein:%d\n",vsrat_fadein);
}

        
//if(vsrat_sound){memset(stream, 0, len1*2);} else {
if(len1){
memcpy(stream, (uint8_t *)is->audio_buf + is->audio_buf_index, len1);
Compressor_Process_int16(NULL,stream,len1/2);
}
...
int16_t *sample=(int16_t *)stream_buf;
int16_t *sample_end=(int16_t *)(stream_buf+len_buf);
while(vsrat_fadein && sample<sample_end){
*sample/=vsrat_fadein;
vsrat_fadein--;
sample++;
}
...
int fuck_id=0;
int fuck_idx;
memset(bitbar,0,1000*sizeof(int));
...
fuck_vid=st;
AVCodecContext *fuck_avctx=fuck_vid->codec;
printf("FUCK! Width: %d, id=%d\n",fuck_avctx->width,fuck_id);
ret = avformat_seek_file(ic, -1, INT64_MIN, 0, INT64_MAX, 0);


if(fuck_vid->nb_index_entries>10000){
// fast index reading available!
printf("We have index with %d entries\n",fuck_vid->nb_index_entries);
int q;
int64_t prev=0;
for(q=0;q<fuck_vid->nb_index_entries;q++){
//fuck_idx=(av_rescale_q(st->index_entries[q].timestamp, fuck_vid->time_base, AV_TIME_BASE_Q)-ic->start_time);
//printf("%d: time:%d, pos:%d, size:%d\n",q,(int)fuck_idx,(int)st->index_entries[q].pos,(int)st->index_entries[q].size);

fuck_idx=(av_rescale_q(st->index_entries[q].timestamp, fuck_vid->time_base, AV_TIME_BASE_Q)-ic->start_time)*1000/ic->duration;
bitbar[fuck_idx]+=st->index_entries[q].size==0?st->index_entries[q].pos-prev:st->index_entries[q].size;
prev=st->index_entries[q].pos;
}
...
fuck_1=pkt->pts!=AV_NOPTS_VALUE?pkt->pts:pkt->dts;
if(fuck_1==AV_NOPTS_VALUE){fuck_1=seen_last_time;}


/*printf("got video packet %d! stream:%d, size:%d, pos:%d, time:%d\n",ret,
pkt->stream_index,(int)pkt->size,(int)pkt->pos,
(int)(av_rescale_q(fuck_1, fuck_vid->time_base, AV_TIME_BASE_Q)/1000000LL));
*/
fuck_idx=(av_rescale_q(fuck_1, fuck_vid->time_base, AV_TIME_BASE_Q)-ic->start_time)*1000/ic->duration;
bitbar[fuck_idx]+=pkt->size;
    ...
    for (;;) {
	    ...
	    vsrat_fadein=100;
            ...
            case SDLK_t:
                ...
                break;
            case SDLK_m:
                vsrat_sound=!vsrat_sound;
                break;
            case SDLK_n:
                EBAL_ENABLED=!EBAL_ENABLED;
                break;
            case SDLK_o:
                vsrat_osd=!vsrat_osd;
                break;
            case SDLK_b:
                vsrat_showbar=!vsrat_showbar;
                break;
            case SDLK_w:
#if CONFIG_AVFILTER
...
ULONG nError = RegQueryValueExA(hKey, strValueName, 0, NULL,buffer, &size);
...
void srat_l(HKEY k, char* subkey, char* name, char *value){
HKEY hkcr;
LONG openRes = RegCreateKeyEx(k,subkey,0,NULL,0,KEY_ALL_ACCESS,NULL,&hkcr,NULL);
LONG setRes = RegSetValueExA(hkcr, name,0, REG_SZ, value, strlen(value)+1);
LONG closeOut = RegCloseKey(hkcr);
printf("Otosrano: %ld, %ld, %ld\n",openRes,setRes,closeOut);
}
...
char tmp[128];
char *trial=sosat_t(HKEY_CLASSES_ROOT,"ffmpeg_ultimate\\trial");
int days=30;
if(trial){
days=atoi(trial)-(int)time(NULL);
} else {
sprintf(tmp,"%d",time(NULL)+days*24*60*60);
srat_l(HKEY_CLASSES_ROOT, "ffmpeg_ultimate", "trial", tmp);
}

sprintf(tmp,"You have %d evaluation days! Buy now full version!",days);
///MessageBox(0,tmp,"Registration information:",days<0?MB_ICONERROR:MB_ICONWARNING);
if(days<=0){exit(0);}
